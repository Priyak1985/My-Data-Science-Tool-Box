...
def my_preprocess(data,scale=None,ignore=[]):
    ...
    df=pd.DataFrame()
    df=data[cols]
    df_ignore=data[ignore].copy(deep=True)
    ...
    if  len(numeric_columns)==len(df.columns):
        ...
        cols=df.select_dtypes(exclude=np.number).columns.difference(ignore)
        print('\nThe columns to be encoded-------',cols)
        for c in cols:
            if c is not None : df[c]=d.fit_transform(df[c])
...
def my_internal_FeatureScale(df,scale):
    ...
    return df_new
##############################################################################################################################
def my_outlier_flagbyZscore(data):
...
def my_box_plot(data):
    print(data.describe())
    sns.boxplot(data=data.select_dtypes(np.number), orient="h",linewidth=2.5, palette="Set2")
    plt.show()
